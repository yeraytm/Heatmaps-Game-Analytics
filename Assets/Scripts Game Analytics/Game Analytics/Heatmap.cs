...
public class Heatmap
{
    ...
    public Heatmap(int width, int height, float cellSize, Vector3 originPosition, Gradient gradient, List<SpatialData> spatialDataList, GameObject cubeGO, bool debugging)
    {
        ...
        if (debugging)
        {
            for (int x = 0; x < gridArray.GetLength(0); x++)
            {
                ...
                {
                    debugTextArray[x, y] = UtilsClass.CreateWorldText(gridArray[x, y].ToString("F2"), null, GetWorldPosition(x, y) + new Vector3(cellSize, 0, cellSize) * .5f, 8, Color.black, TextAnchor.MiddleCenter);
    ...
    public void SetValue(int x, int y, float value)
    {
        if (x >= 0 && y >= 0 && x < width && y < height)
        {
            ...
            if (debugging)
                debugTextArray[x, y].text = gridArray[x, y].ToString("F2");
        }
    }
